...
const Spends = () => {
  ...
  useEffect(() => {
    ...
    const sub = DeviceEventEmitter.addListener('expenseRefresh', () => {
      fetchExpenses();
    });
    return () => sub.remove();
  }, []);
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const userIdResponse = await fetch(`${SERVER_BASE_URL}/auth/v1/ping`, {
        method: 'GET',
        headers: {
      ...
      const userId = (await userIdResponse.text()).trim();
